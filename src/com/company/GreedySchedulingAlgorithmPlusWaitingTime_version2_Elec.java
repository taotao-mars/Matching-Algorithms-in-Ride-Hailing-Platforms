...
public class GreedySchedulingAlgorithmPlusWaitingTime_version2_Elec extends TripSchedulingAlgorithm implements TripSchedulingAlgorithm.algorithm {
    ...
    public static int TripSchedule(Car_list car_list, Trip trip, int max_waiting_time){
        ...
        double e_last_trip_remain_time=0;
        int temp=0;
        int sign=0;
        int e_temp=0;
        int e_sign=0;
        ...
        while(temp<size-e_size){
            last_trip_remain_time=Math.max(car_list.car_list.get(index[temp]+e_size).getTime_stamp_arrive_des()-trip.getStartTime(),0);
        ...
        while(e_temp<e_size){
            e_last_trip_remain_time=Math.max(car_list.car_list.get(e_index[e_temp]).getTime_stamp_arrive_des()-trip.getStartTime(),0);
            if(e_a[e_temp]>0&&e_last_trip_remain_time+e_a[e_temp]<max_waiting_time){
                e_sign=1;
                break;
            }
            e_temp++;
        ...
        if(sign==0&&e_sign==0){
            ...
            if(e_last_trip_remain_time+e_a[e_temp]-last_trip_remain_time-a[temp]>600){
                car_index=index[temp]+e_size;
                pick_up_waiting_time=a[temp];
            }else{
                car_index=e_index[e_temp];
                pick_up_waiting_time=e_a[e_temp];
                last_trip_remain_time=e_last_trip_remain_time;
            }
        }else{
            if(sign==1){
                car_index=index[temp]+e_size;
        ...
        trip.setTimestamp_picked_up(pick_up_waiting_time,last_trip_remain_time);
